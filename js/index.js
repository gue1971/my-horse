...
function buildCard({ horse, hero, hasAlbum }) {
  ...
  clubImg.src = `assets/icons/clubs/${horse.club}.gif`;
  ...
  const price = (horse.price || '') + '募集';
  const birthAndPrice = `${birthDate}　${price}`; // 全角スペース
  mini.appendChild(mkRow([['', birthAndPrice]]));
  ...
  jraLink.target = '_blank'; jraLink.rel = 'noopener';
  jraLink.innerHTML = '<img src="assets/icons/jra.png" alt="JRA">JRA';
  if (!horse.jra_id) { jraLink.style.pointerEvents = 'none'; jraLink.style.opacity = '0.5'; }

  const jbisLink = document.createElement('a');
  jbisLink.href = horse.jbis_id ? jbisUrl(horse.jbis_id) : '#';
  jbisLink.target = '_blank'; jbisLink.rel = 'noopener';
...
function showTab(id){
  ...
  // フッターのドット状態を更新
  $$('.dot').forEach(b=>b.classList.toggle('is-active', b.dataset.target===id));
